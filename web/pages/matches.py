...
def render(matches, players, names, params=None, iso="", now=None, total=None,
    ...
    body.append('<section class="wrap rise rise-1">'
                ...
                + _days(shown, names, now, _view_of(params))
                + "</section>")
...
def _days(matches, names, now, view):
    if not matches:
    ...
    if now is None:
        return f'<div class="matches">{_cards(matches, names, view)}</div>'
    out = []
    for label, day, group in derive.group_by_day(matches, now):
        ...
        out.append(f'<div class="day"><div class="day-head">'
                   ...
                   f'<div class="matches">{_cards(group, names, view)}</div></div>')
    return f'<div class="day-list">{"".join(out)}</div>'


def _cards(matches, names, view=derive.OVERALL):
    return "".join(c.match_card(blob, names, view) for blob in matches)
